...
import { Crosshair, ButterflyIcon, RoseIcon, TulipIcon, BlueElement } from '../icons/index';
...
const Background: React.FC = () => {
  ...
  const tulips = Array.from({ length: 7 }).map((_, i) => ({
    id: `t-${i}`,
    x: Math.random() * 100,
    y: Math.random() * 100,
    size: Math.random() * 20 + 15,
  ...
  const blueElements = Array.from({ length: 5 }).map((_, i) => ({
    id: `b-el-${i}`,
    x: Math.random() * 100,
    y: Math.random() * 100,
